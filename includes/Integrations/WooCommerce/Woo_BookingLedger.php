...
class Woo_BookingLedger {
	...
	/**
	 * Initialize postmortem probe for TCBF-14 debugging
	 *
	 * Captures the exact source of string keys when the crash occurs:
	 * - blocks_bad_keys: from get_blocks_in_range()
	 * - existing_weird_starts: from existing bookings with string start values
	 *
	 ...
	 */
	private static function init_postmortem_probe() : void {
		add_action( 'plugins_loaded', function() {
			...
			set_error_handler( function( $errno, $errstr, $errfile, $errline ) use ( $logger, $context ) {
				...
				$logger->error( 'TCBF-14 CRASH CAUGHT!', array_merge( $context, [
				...
				if ( function_exists( 'WC' ) && WC()->cart ) {
					...
					$logger->error( 'TCBF-14 CRASH: Cart state', array_merge( $context, [
			...
			// Shutdown handler with POSTMORTEM PROBE
			register_shutdown_function( function() use ( $logger, $context ) {
				...
				$logger->error( 'TCBF-14 FATAL via shutdown', array_merge( $context, [
				...
				// --- TCBF-14 POSTMORTEM PROBE: reconstruct Bookings inputs after fatal ---
				try {
					if ( function_exists( 'WC' ) && WC()->cart ) {
						$probe = [];
						...
						foreach ( WC()->cart->get_cart() as $cart_key => $item ) {
							...
							$start       = $booking['_start_date'] ?? null;
							$end         = $booking['_end_date'] ?? null;
							$resource_id = isset( $booking['_resource_id'] ) ? absint( $booking['_resource_id'] ) : 0;
							...
							$entry = [
								...
								'blocks_keys_head'      => [],
								'blocks_bad_keys'       => [],
								'existing_ids_count'    => null,
								'existing_weird_starts' => [],
							];

							// 1) Are blocks already poisoned?
							try {
								if ( method_exists( $product, 'get_blocks_in_range' ) ) {
									$blocks = $product->get_blocks_in_range( $start, $end, [], $resource_id );
									$keys   = array_slice( array_keys( (array) $blocks ), 0, 80 );
									...
									$entry['blocks_keys_head'] = $keys;
									$entry['blocks_bad_keys']  = $bad;
								} else {
									$entry['blocks_bad_keys'] = [ 'no_get_blocks_in_range_method' ];
								}
							} catch ( \Throwable $e ) {
								$entry['blocks_bad_keys'] = [ 'get_blocks_in_range_threw' => $e->getMessage() ];
							}

							// 2) Do any "existing bookings" return string starts?
							try {
								$query_object = ( method_exists( $product, 'has_resources' ) && $product->has_resources() && $resource_id )
									? $resource_id
									: $product->get_id();

								$ids = [];
								if ( class_exists( 'WC_Booking_Data_Store' ) ) {
									$ids = \WC_Booking_Data_Store::get_bookings_in_date_range(
										$start + 1,
										$end - 1,
										$query_object,
										true
								...
								foreach ( (array) $ids as $bid ) {
									...
									$gs = $b->get_start();
									if ( is_int( $gs ) || ( is_string( $gs ) && ctype_digit( $gs ) ) ) {
										continue;
									}
								...
								$entry['existing_weird_starts'] = $weird;

							} catch ( \Throwable $e ) {
								$entry['existing_weird_starts'] = [ 'get_bookings_in_date_range_threw' => $e->getMessage() ];
							}

							$probe[] = $entry;
						}

						$logger->error( 'TCBF-14 POSTMORTEM PROBE', array_merge( $context, [ 'probe' => $probe ] ) );
					}
				} catch ( \Throwable $e ) {
					$logger->error( 'TCBF-14 POSTMORTEM PROBE FAILED', array_merge( $context, [ 'msg' => $e->getMessage() ] ) );
				}
			} );
