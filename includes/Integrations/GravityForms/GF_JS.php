...
final class GF_JS {
	...
	private static function build_partner_override_js( int $form_id, array $partners, string $initial_code = '' ) : string {
		...
		return "window.tcBfPartnerMap = window.tcBfPartnerMap || {};\n"
			...
			. "          // ISSUE: Product price sub-inputs (137.2, 138.2, etc.) don't exist in DOM.\n"
			. "          // Single Product fields don't render .2 price inputs - they're only used in formulas.\n"
			. "          // WORKAROUND: Apply partner discount to total (not per-scope). PHP validation will fix it.\n"
			. "          // Read EB % and subtotal\n"
			. "          var ebPct = parseLocaleFloat((qs('#input_'+fid+'_172')||{}).value||0);\n"
			. "          var subtotal = parseLocaleFloat((qs('#input_'+fid+'_173')||{}).value||0);\n"
			. "          console.log('[partner_disc] Using simplified calculation: subtotal='+subtotal+', ebPct='+ebPct);\n"
			. "          if(subtotal <= 0) { console.log('[partner_disc] No subtotal, using default'); return result; }\n"
			. "          // Calculate EB amount\n"
			. "          var ebAmount = Math.round((subtotal * (ebPct/100) + 0.000000001) * 100) / 100;\n"
			. "          // Calculate base after EB\n"
			. "          var baseAfterEB = Math.max(0, subtotal - ebAmount);\n"
			. "          // Calculate partner discount on total base (not per-scope)\n"
			. "          var partnerDiscount = Math.round((baseAfterEB * (partnerPct/100) + 0.000000001) * 100) / 100;\n"
			. "          console.log('[partner_disc] Calculated: ebAmount='+ebAmount+', baseAfterEB='+baseAfterEB+', partnerDiscount='+partnerDiscount);\n"
			. "          console.log('[partner_disc] Returning (simplified):', String(partnerDiscount.toFixed(2)));\n"
			. "          console.warn('[partner_disc] NOTE: This is simplified calculation. PHP server-side validation will apply correct per-scope rounding.');\n"
			. "          return String(partnerDiscount.toFixed(2));\n"
