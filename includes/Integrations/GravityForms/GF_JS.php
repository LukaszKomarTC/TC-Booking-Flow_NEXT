...
final class GF_JS {
	...
	private static function build_partner_override_js( int $form_id, array $partners, string $initial_code = '' ) : string {
		...
		return "window.tcBfPartnerMap = window.tcBfPartnerMap || {};\n"
			...
			. "    if(!field181) return true; // Default to enabled if field missing (safety fallback)\n"
			. "    var val = (field181.value||'').toString().trim();\n"
			. "    return val !== '0';\n"
			. "  }\n"
			. "  function applyPartner(){\n"
			. "    // TCBF-12: Check if partner program is enabled for this event\n"
			. "    if(!isPartnerProgramEnabled()){\n"
			. "      // Hide partner fields\n"
			. "      hideField(63);  // Admin partner override dropdown\n"
			. "      hideField(154); // Coupon code input\n"
			. "      hideField(176); // Partner discount display\n"
			. "      hideField(165); // Partner summary section\n"
			. "      // Clear partner data fields\n"
			. "      setVal(154,''); setVal(152,''); setVal(161,''); setVal(153,''); setVal(166,''); setVal(176,'0');\n"
			. "      // Recalculate totals without partner discount\n"
			...
			. "  // Cache for fields 174/175 values (since DOM inputs may not be accessible)\n"
			. "  var cachedPartTotal = 0;\n"
			. "  var cachedRentalTotal = 0;\n"
			...
			. "          // Cache field 174 (participation total) and 175 (rental total) results\n"
			. "          if(fieldId === 174){ cachedPartTotal = parseFloat(result) || 0; console.log('[partner_disc] Cached partTotal:', cachedPartTotal); return result; }\n"
			. "          if(fieldId === 175){ cachedRentalTotal = parseFloat(result) || 0; console.log('[partner_disc] Cached rentalTotal:', cachedRentalTotal); return result; }\n"
			. "          if(fieldId !== 176) return result; // Only intercept partner discount field\n"
			. "          // TCBF-12: If partner program disabled, force discount to 0\n"
			. "          if(!isPartnerProgramEnabled()) { console.log('[partner_disc] Partner program disabled (field 181), forcing discount to 0'); return 0; }\n"
			. "          // Read partner %\n"
			...
			. "          // Read EB % and use cached scope totals (fields 174/175)\n"
			. "          var ebPct = parseLocaleFloat((qs('#input_'+fid+'_172')||{}).value||0);\n"
			. "          var partTotal = cachedPartTotal;\n"
			. "          var rentalTotal = cachedRentalTotal;\n"
			. "          console.log('[partner_disc] Per-scope calculation: partTotal='+partTotal+', rentalTotal='+rentalTotal+', ebPct='+ebPct);\n"
			. "          console.log('[partner_disc] Using cached values (fields 174/175 calculated before 176)');\n"
