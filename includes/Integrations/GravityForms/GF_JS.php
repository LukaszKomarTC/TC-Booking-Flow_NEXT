...
final class GF_JS {
	...
	private static function build_partner_override_js( int $form_id, array $partners, string $initial_code = '' ) : string {
		...
		return "window.tcBfPartnerMap = window.tcBfPartnerMap || {};\n"
			...
			. "          // Read EB %\n"
			. "          var ebPct = parseLocaleFloat((qs('#input_'+fid+'_172')||{}).value||0);\n"
			. "          // Read participation prices (fields 137.2, 138.2)\n"
			. "          var inp137 = qs('#input_'+fid+'_137_2');\n"
			. "          var inp138 = qs('#input_'+fid+'_138_2');\n"
			. "          console.log('[partner_disc] inp137 exists:', !!inp137, 'value:', inp137 ? inp137.value : 'N/A');\n"
			. "          console.log('[partner_disc] inp138 exists:', !!inp138, 'value:', inp138 ? inp138.value : 'N/A');\n"
			. "          var partPrice = parseLocaleFloat((inp137||{}).value||0);\n"
			. "          var partMemberPrice = parseLocaleFloat((inp138||{}).value||0);\n"
			. "          var partTotal = partPrice + partMemberPrice;\n"
			. "          // Read rental prices (fields 139.2, 140.2, 141.2, 171.2)\n"
			. "          var inp139 = qs('#input_'+fid+'_139_2');\n"
			. "          var inp140 = qs('#input_'+fid+'_140_2');\n"
			. "          var inp141 = qs('#input_'+fid+'_141_2');\n"
			. "          var inp171 = qs('#input_'+fid+'_171_2');\n"
			. "          console.log('[partner_disc] inp139:', !!inp139, inp139 ? inp139.value : 'N/A');\n"
			. "          console.log('[partner_disc] inp140:', !!inp140, inp140 ? inp140.value : 'N/A');\n"
			. "          console.log('[partner_disc] inp141:', !!inp141, inp141 ? inp141.value : 'N/A');\n"
			. "          console.log('[partner_disc] inp171:', !!inp171, inp171 ? inp171.value : 'N/A');\n"
			. "          var rentalRoad = parseLocaleFloat((inp139||{}).value||0);\n"
			. "          var rentalMTB = parseLocaleFloat((inp140||{}).value||0);\n"
			. "          var rentalEMTB = parseLocaleFloat((inp141||{}).value||0);\n"
			. "          var rentalGravel = parseLocaleFloat((inp171||{}).value||0);\n"
			. "          var rentalTotal = rentalRoad + rentalMTB + rentalEMTB + rentalGravel;\n"
			. "          console.log('[partner_disc] Inputs: ebPct='+ebPct+', partTotal='+partTotal+', rentalTotal='+rentalTotal);\n"
			. "          // Calculate bases after EB (proportional distribution like PHP)\n"
			. "          var subtotalOriginal = partTotal + rentalTotal;\n"
			. "          if(subtotalOriginal <= 0) return result;\n"
			. "          var ebAmount = 0;\n"
			. "          if(ebPct > 0){\n"
			. "            ebAmount = Math.round((subtotalOriginal * (ebPct/100) + 0.000000001) * 100) / 100;\n"
			. "          }\n"
			. "          // Distribute EB proportionally\n"
			...
			. "          var partBaseAfterEB = Math.max(0, partTotal - ebAmtPart);\n"
			. "          var rentalBaseAfterEB = Math.max(0, rentalTotal - ebAmtRental);\n"
			. "          var discPart = Math.round((partBaseAfterEB * (partnerPct/100) + 0.000000001) * 100) / 100;\n"
			. "          var discRental = Math.round((rentalBaseAfterEB * (partnerPct/100) + 0.000000001) * 100) / 100;\n"
			. "          var totalDisc = discPart + discRental;\n"
			. "          console.log('[partner_disc] partBaseAfterEB='+partBaseAfterEB+', rentalBaseAfterEB='+rentalBaseAfterEB);\n"
			. "          console.log('[partner_disc] discPart='+discPart+', discRental='+discRental+', totalDisc='+totalDisc);\n"
			. "          console.log('[partner_disc] Returning:', String(totalDisc.toFixed(2)));\n"
			. "          // Return as dot-decimal string (GF will format to locale)\n"
