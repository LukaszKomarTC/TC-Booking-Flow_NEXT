...
final class GF_JS {
	...
	private static function build_partner_override_js( int $form_id, array $partners, string $initial_code = '', array $i18n = [] ) : string {
		...
		return "window.tcBfPartnerMap = window.tcBfPartnerMap || {};\n"
			...
			. "    }\n"
			. "  }\n"
			. "  function enhanceField180(data, code){\n"
			. "    var field180 = qs('#field_'+fid+'_180');\n"
			. "    if(!field180) return;\n"
			. "    var container = qs('.ginput_container', field180);\n"
			. "    if(!container) return;\n"
			...
			. "      var wrapper = qs('.ginput_product_price_wrapper', field180);\n"
			. "      if(wrapper) wrapper.style.display = 'none';\n"
			. "    }\n"
			. "    if(data && code && data.discount){\n"
			. "      var amountSpan = qs('#input_'+fid+'_180');\n"
			. "      var amount = amountSpan ? amountSpan.textContent : '';\n"
			. "      enhanced.innerHTML = '<div class=\"tcbf-partner-badge\"><span class=\"tcbf-partner-icon\">✓</span><span class=\"tcbf-partner-code\">'+code.toUpperCase()+'</span></div><div class=\"tcbf-partner-info\"><div class=\"tcbf-partner-pct\">'+fmtPct(data.discount)+'% '+discountLabel+'</div><div class=\"tcbf-partner-amt\">'+amount+'</div></div>';\n"
