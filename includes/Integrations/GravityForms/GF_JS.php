...
final class GF_JS {
	...
	private static function build_partner_override_js( int $form_id, array $partners, string $initial_code = '' ) : string {
		...
		return "window.tcBfPartnerMap = window.tcBfPartnerMap || {};\n"
			...
			. "    // TEMPORARY: Always return true to bypass toggle check\n"
			. "    return true;\n"
			. "    /*\n"
			...
			. "    // Safety: if field has parameter name instead of value, default to enabled\n"
			. "    if(val === 'partners_enabled' || val === '') return true;\n"
			. "    return val !== '0';\n"
			. "    */\n"
			. "  }\n"
			...
			. "      try{ window.gformCalculateTotalPrice(fid); }catch(e){}\n"
			. "    }\n"
			. "    }finally{ applyPartnerInProgress = false; }\n"
			. "  }\n"
			. "  // ---- Partner discount rounding parity (GF vs Woo) ----\n"
			. "  // GF calculated field 176: ({Base after EB:174} * {Partner Discount %:152} / 100)\n"
			. "  // GF rounds half-up, while Woo discount totals effectively round DOWN.\n"
